...
**`Memory` facade** (`src/memo/memory/facade.py`) multiply-inherits eighteen operation mixins — `_WriteOpsMixin`, `_UpdateOpsMixin`, `_RelationOpsMixin`, `_LifecycleOpsMixin`, `_DeleteOpsMixin`, `_GraphOpsMixin`, `_SearchOpsMixin`, `_SearchScoringMixin`, `_EvidenceOpsMixin`, `_OutcomeFeedbackOpsMixin`, `_AskOpsMixin`, `_ChatAskOpsMixin`, `_RerankOpsMixin`, `_RepoOpsMixin`, `_MaintainOpsMixin`, `_ConsolidateOpsMixin`, `_ReplayOpsMixin`, `_SecretOpsMixin` — each in their own `src/memo/memory/<op>_ops.py` file. Module-level constants, prompts, and pure helpers are in `src/memo/memory/record.py`. Never import from a mixin directly; always go through `Memory`.
...
**Flags** (`src/memo/flags.py`) is the single registry for all `MEMO_*` env vars — it aggregates `FlagSpec`s defined in the per-domain `flags_<group>.py` modules (`flags_recall/search/behavior/capture/ingest/graph/misc.py`, base types in `flags_base.py`). Add a new flag in the matching `flags_<group>.py`. Use `flag_bool/int/float/str(name)` — never `os.environ.get("MEMO_...")` inline. `memo config validate` parses every set flag.
...
**Memory types** (`src/memo/tiers.py`): durable tier = `decision`, `fact`, `bug`, `feedback`, `preference`, `note`, `manual`, `synthesis`, `procedure`, `failure_pattern`; reference tier = `reference` (bulk-ingested vault chunks, excluded from auto-recall). `synthesis` memories are auto-generated by `memo synthesize` / `MEMO_SYNTHESIS_ENABLED=1 memo maintain` — cross-cluster inferred insights with `synthesis_sources` provenance in their `extra` frontmatter bag.
...
(wired by `wire_recall_hook()` in `src/memo/cli_hooks.py`, which writes the
entry into `~/.claude/settings.json` — NOT `hooks/hooks.json`, which owns the
session/capture/sync/maintenance hooks only). End-to-end it must stay under ~5s. Cold MLX load is ~2s,
...
under the timeout. Anything added to the hook path — `src/memo/cli_hooks.py`, the
`recall-hook` command in `src/memo/cli_recall_hook.py`, `embedder.py`, `store/queries.py` search —
...
chain, though the 11 names are excluded from `flags.unknown_memo_vars` so
...
Bump the version in sync across **twelve** versioned files plus `CHANGELOG.md`:
...
- Gate (fast, no MLX — retrieval only, ~0.5s/prompt):
  ...
  across ALL prompts (the label set grows with every incident — 46 prompts as of
  2026-08; the authoritative numbers are the per-machine saved baseline below,
...
- **Dream v2 — self-improvement passes (opt-in per flag; all default OFF
  EXCEPT `MEMO_DREAM_FLAG_GRADUATION_ENABLED`, which has defaulted ON since
  v4.3.0):**
...
- Already hold a memory id or a named entity and want what's connected to
  it? Reach for `memo_graph` (cheap graph traversal — id/title only; the
  richer `memo_related` exists only on the full/default MCP profile) before
  firing a fresh `memo_search`/`memo_ask` (full retrieval) for the same thing.
...
- Keep it honest: when a surfaced memory is stale or contradicted, correct it
  ...
  reversible and never hard-delete. (`memo_feedback_flag` does the same thing
  but is only registered on the full/default MCP profile, not on `agent`/`core`.)
<!-- /memo-mandate -->
