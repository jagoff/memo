...
If MCP tools named `mcp__memo__memory_*` are not available, tell the user to
register memo first:
...
Then run the printed `codex mcp add ...` command and restart the session.

## Routing

- Empty arguments: smart-capture the actionable insight from the current turn
  and call `mcp__memo__memory_save`.
