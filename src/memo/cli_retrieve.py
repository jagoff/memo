"""Retrieve crushed JSON from crush cache via CLI.

Wave 1 token economy: L1 SmartCrusher offloads low-relevance JSON rows to cache.
This command retrieves the original (full) JSON using the marker hash.

Usage: memo retrieve <<memo-crush:HASH>>
Output: JSON {"original": <original_json_string>, "hash": <hash>}
"""

from __future__ import annotations

import json

import click

from memo.config import Config
from memo.store.crush_cache import CrushCache


@click.command()
@click.argument("hash_marker", type=str)
def retrieve_cmd(hash_marker: str) -> None:
    """Retrieve original JSON from crush cache.

    Takes a crush marker (e.g., <<memo-crush:abc123def456>>) and returns
    the original JSON that was offloaded. Hash is extracted from the marker.

    Returns JSON with "original" (the full JSON string) and "hash" fields.
    """
    # Parse marker format: <<memo-crush:HASH>> -> extract HASH
    if not hash_marker.startswith("<<memo-crush:") or not hash_marker.endswith(">>"):
        click.echo("Error: invalid marker format. Expected: <<memo-crush:HASH>>", err=True)
        return

    hash_val = hash_marker[13:-2]  # Strip <<memo-crush: and >>

    config = Config.from_env()
    cache = CrushCache(config.state_dir)

    original = cache.retrieve(hash_val)
    if original is None:
        click.echo(f"Error: cache entry not found or expired: {hash_val}", err=True)
        return

    # Output as JSON for easy shell piping
    click.echo(json.dumps({"original": original, "hash": hash_val}, ensure_ascii=False))
