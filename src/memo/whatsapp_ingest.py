...
def _managed_chat_jid(path: Path) -> str | None:
    """Return the chat JID only for a regular note generated by this importer."""
    ...
    try:
        ...
        return None
    if str(post.metadata.get("source") or "").strip().casefold() != "whatsapp":
        return None
    jid = str(post.metadata.get("chat_jid") or "").strip()
    return jid or None
