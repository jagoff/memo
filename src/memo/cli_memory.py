...
# Durable types a human may set from the CLI. Derived from the registry so the
# CLI cannot drift from it again — the hardcoded list silently lacked
# `procedure` and `failure_pattern` long after they became durable types.
# `synthesis` stays out: it is machine-generated by the synthesize pass.
_USER_SETTABLE_TYPES = sorted(DURABLE_TYPES - {"synthesis"})
...
@click.option(
    "--type",
    "type_",
    type=click.Choice(_USER_SETTABLE_TYPES),
    default="note",
    show_default=True,
)
