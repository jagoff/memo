...
SPECS: tuple[FlagSpec, ...] = (
    ...
    _spec(
        ...
        False,
        "search",
        "Enable unified graph signal collection for retrieval. Default off so graph ranking effects stay eval-gated.",
    ),
    _spec(
        ...
        False,
        "search",
        "Attach graph_reason attribution to graph-touched search results.",
    ),
    _spec(
        ...
        False,
        "search",
        "Enable deterministic semantic relation storage and relation reads from the graph sidecar.",
    ),
    _spec(
        ...
        True,
        "search",
        "Suppress high-document-frequency graph hubs when computing graph retrieval signal.",
    ),
    _spec(
        ...
        150,
        "search",
        "Maximum milliseconds for graph signal work in hot retrieval paths.",
        min_val=0,
    ),
    _spec(
        ...
        0.25,
        "search",
        "Entity document-frequency ratio above which an entity is treated as a graph hub.",
        min_val=0.0,
        max_val=1.0,
    ),
    _spec(
        ...
        0.5,
        "search",
        "Minimum query entity IDF required before graph signal affects ranking.",
        min_val=0.0,
    ),
    _spec(
