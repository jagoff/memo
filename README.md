...
**Prompts and memories stay on your machine.** Embedder, reranker, and LLM all run in-process. No telemetry. Memory travels only if *you* point `memo sync` at a git remote you own. A throttled update tag check is on by default; set `MEMO_AUTO_UPDATE=0` for fully offline startup. → **[Privacy and network policy](https://github.com/jagoff/memo/blob/master/PRIVACY.md)**
