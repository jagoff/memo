"""Regression: `memo_repo_index` must not reach git's arbitrary-command remote
helpers (`ext::`/`fd::`) via a caller-supplied URL — that is an RCE from the MCP
surface. See `_validate_clone_url` in repo_index_helpers.py."""

from __future__ import annotations

import pytest

from memo.repo_index_helpers import _GIT_SAFE_ENV, _validate_clone_url
...
@pytest.mark.parametrize(
    "url",
    [
        "https://github.com/jagoff/memo.git",
        "http://example.com/x.git",
        "ssh://git@github.com/jagoff/memo.git",
        "git://example.com/x.git",
        "git+ssh://git@host/x.git",
        "file:///Users/fer/repos/memo",
        "git@github.com:jagoff/memo.git",  # scp-like
        "user.name@host.example.com:group/repo.git",
    ],
)
