...
from memo.proactive.surfaces import render_badge, render_digest, render_urgent_line
...
def test_badge_empty_when_no_candidates():
    assert render_badge(Routed(0, None, [], None)) == ""


def test_badge_reliability_icon():
    assert render_badge(Routed(3, KIND_RELIABILITY, [], None)) == "⚠️3"
...
def test_digest_empty_message():
    assert "nothing to surface" in render_digest(Routed(0, None, [], None))


def test_digest_groups_kinds():
    routed = Routed(
        2, KIND_RELIABILITY, [_n(KIND_RELIABILITY, "a"), _n(KIND_CONTINUITY, "b")], None
    )
