...
def test_interject_fires_only_on_high_band_and_disputed():
    ...
    cands = ij.interject_candidates(
        "let's switch to hybrid instead", hits,
        sim_threshold=0.6, band_of=lambda h: band[h.id], disputed_ids=disputed,
    )
...
def test_interject_empty_without_reversal_signal():
    ...
    cands = ij.interject_candidates(
        "how does recall work", hits,
        sim_threshold=0.6, band_of=lambda h: "high", disputed_ids={"a" * 32},
    )
    assert cands == []
...
def test_interject_banner_none_when_not_disputed():
    ...
    banner = ij.interject_banner(
        "switch instead", hits,
        sim_threshold=0.6, band_of=lambda h: "high", disputed_ids=set(),
    )
