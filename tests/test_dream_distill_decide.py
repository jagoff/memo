...
def _items():
    return [
        {"id": "a" * 32, "title": "Ta", "type": "decision", "tags": [], "path": "p", "updated": "2026-06-01", "emb": []},
        {"id": "b" * 32, "title": "Tb", "type": "fact", "tags": [], "path": "p", "updated": "2026-06-01", "emb": []},
        {"id": "c" * 32, "title": "Tc", "type": "note", "tags": [], "path": "p", "updated": "2026-06-01", "emb": []},
    ]
...
def test_assemble_clusters_carries_maturity_and_drops_singletons():
    ...
    clusters = dd.assemble_clusters(
        items, [[0, 1], [2]],
        health=health, support=support, created_by_id=created, min_cluster=2, now=_now(),
    )
...
def test_decide_saves_mature_and_skips_immature():
    ...
    out = dd.decide_distillations(
        clusters, synthesize_fn=synth, exists_fn=never_exists, is_mature_fn=mature,
        dry_run=False, max_clusters=5,
    )
...
def test_decide_dedups_by_provenance():
    ...
    out = dd.decide_distillations(
        clusters, synthesize_fn=lambda cl: {"title": "x", "body": "y"},
        exists_fn=lambda _p: True, is_mature_fn=lambda s: True, dry_run=False, max_clusters=5)
...
def test_decide_dry_run_saves_nothing():
    ...
    out = dd.decide_distillations(
        clusters, synthesize_fn=lambda cl: {"title": "x", "body": "y"},
        exists_fn=lambda _p: False, is_mature_fn=lambda s: True, dry_run=True, max_clusters=5)
