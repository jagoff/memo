...
def test_compute_roi_no_fabricated_tokens_estimate(tmp_path: Path) -> None:
    """The old `tokens_saved` field (grounded * hardcoded constant) is gone —
    `memo tokens` reports the real, measured cost/savings instead. Only the
    real, measured avg-answer-tokens transparency number remains."""
    _recall(tmp_path, "s", 1, "configure the deploy pipeline yaml settings now", "mem00001")
    ...
    assert "tokens_per_grounded" not in data
