...
def test_rerank_applies_distance_decay():
    """When MEMO_GRAPH_DISTANCE_DECAY=True, distant memories score lower."""
    cfg = Config(data_dir="/tmp/test_memo", state_dir="/tmp/test_memo_state")
    memory = Memory(cfg)
    ...
    with patch.object(memory.graph, "distance_to_nearest_fact") as mock_dist:
        ...
        # Patch flags
        with patch("memo.memory.rerank_ops.flag_bool", return_value=True):
            with patch("memo.memory.rerank_ops.flag_float", return_value=0.15):
...
def test_distance_decay_disabled_no_change():
    """When MEMO_GRAPH_DISTANCE_DECAY=False, scores unchanged."""
    cfg = Config(data_dir="/tmp/test_memo2", state_dir="/tmp/test_memo2_state")
    memory = Memory(cfg)
