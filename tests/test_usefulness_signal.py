...
def test_dedup_double_fire_collapses_subprocess_daemon_pair() -> None:
    ...
    rows = [
        {
            "ts": "2026-05-30T06:11:10+00:00",
            "prompt": "pushea primero",
            "via": "daemon",
            "hits": [{"id": "8d34571e", "score": 0.97}],
        },
        {
            "ts": "2026-05-30T06:11:07+00:00",
            "prompt": "pushea primero",
            "via": "subprocess",
            "hits": [{"id": "976266c7", "score": 0.95}],
        },
        {
            "ts": "2026-05-30T06:10:01+00:00",
            "prompt": "que queda pendiente",
            "via": "daemon",
...
def test_referenced_rate_counts_only_post_surfacing_fetches(tmp_path) -> None:
    ...
    append_recall_log(
        ...
        hits=[
            {"id": "dec00001", "score": 0.9, "title": "Cache decision"},
    ...
    ref = referenced_rate(
        tmp_path,
        [
            {"ts": "2026-05-30T06:00:00+00:00", "hits": [{"id": "dec00001"}, {"id": "fact0001"}]},
        ],
    )
...
def test_consult_breakdown_exposes_strong_hit_rate(tmp_path) -> None:
    append_recall_log(
        ...
        hits=[{"id": "a1", "score": 0.9, "title": "A"}],
        via="daemon",
    )
    append_recall_log(
        tmp_path,
        prompt="beta distinct prompt",
        hits=[{"id": "b2", "score": 0.5, "title": "B"}],
        via="subprocess",
    )
...
def test_consult_breakdown_grounded_rate_excludes_unscored_turns(tmp_path) -> None:
    ...
    append_recall_log(
        tmp_path,
        prompt="alpha what did we decide",
        hits=[{"id": "m1", "score": 0.9, "title": "M1"}],
        via="daemon",
        session_id="s1",
        turn=1,
    )
    append_grounding_log(
        tmp_path, session_id="s1", turn=1, recall_id="m1", used_score=0.9, method="cosine"
    )
    # turn 2: surfaced + scored + NOT used
    append_recall_log(
        tmp_path,
        prompt="beta how does caching work",
        hits=[{"id": "m2", "score": 0.88, "title": "M2"}],
        via="daemon",
        session_id="s1",
        turn=2,
    )
    append_grounding_log(
        tmp_path, session_id="s1", turn=2, recall_id="m2", used_score=0.1, method="cosine"
    )
    # turn 3: surfaced but NEVER scored (no grounding row) → unmeasured, must be excluded
    append_recall_log(
        tmp_path,
        prompt="gamma why is this so slow",
        hits=[{"id": "m3", "score": 0.7, "title": "M3"}],
        via="daemon",
        session_id="s1",
        turn=3,
    )
