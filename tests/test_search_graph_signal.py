...
def test_search_graph_reason_includes_semantic_relations_when_enabled(
    ...
    rec_a = mem.save(
        content="MLX daemon relation note with graph search marker",
        title="MLX Relation Daemon",
        type_="note",
    )
    rec_b = mem.save(
        content="MLX relation project note with graph search marker and separate body",
        title="MLX Relation Hub",
        type_="note",
    )
    mem.graph.record_extraction(
        memory_id=rec_a.id,
